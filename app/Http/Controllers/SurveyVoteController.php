...
use App\Models\Vote;
use App\Models\User;
use App\Models\CategoryCharacter; // Modelo pivote para actualizar ratings ELO
use App\Models\CharacterSurvey; // Modelo pivote para estadísticas por encuesta
use App\Models\SurveyUser; // Modelo pivote para progreso del usuario
...
class SurveyVoteController extends Controller
{
     ...
     * Implementa una transacción atómica para garantizar la consistencia de los datos
     * al actualizar múltiples tablas relacionadas.
     *
     * @param StoreVoteRequest $request // Request validado con reglas específicas para votos
    ...
    public function store(StoreVoteRequest $request, int $surveyId): RedirectResponse
    {
        ...
        // 2. Obtener y validar los datos del voto desde el request
        // StoreVoteRequest ya ha validado winner_id, loser_id, tie, combinatoric_id
        ...
        // 3. Validación de lógica de negocio adicional (después de la validación básica del Request)
        if ($tie) {
            // En caso de empate, winner_id y loser_id deben ser null o ausentes
            // StoreVoteRequest debería garantizar esto con 'required_without:tie'
            // pero revalidamos por seguridad.
            if ($winnerId !== null || $loserId !== null) {
                return back()->withErrors(['tie' => 'If tie is selected, winner and loser must be absent.'])->withInput();
            }
        } else {
            // Si no es empate, winner_id y loser_id son obligatorios y deben ser diferentes
            if (!$winnerId || !$loserId) {
        ...
        // 4. Cargar datos críticos en una sola consulta con JOINs para evitar N+1
        ...
        $surveyData = Survey::with([
                ...
                'combinatorics' => function ($query) use ($combinatoricId) {
                    $query->where('id', $combinatoricId) // Filtrar por la combinación específica
                          ->with([
                              'character1:id,fullname,picture', // Cargar solo campos necesarios del personaje 1
                              'character2:id,fullname,picture'  // Cargar solo campos necesarios del personaje 2
                          ]);
                }
            ])
            ->where('id', $surveyId)
        ...
        // 6. Verificar si el usuario ya votó por esta combinación específica (fuera de la transacción principal para lectura)
        ...
        // 7. Verificar el estado del progreso del usuario (fuera de la transacción principal para lectura)
        ...
        // 8. Determinar el resultado del voto y los personajes involucrados
        ...
        // 9. Cargar ratings ELO de los personajes en la categoría de la encuesta (fuera de la transacción principal también)
        ...
        DB::transaction(function () use (
            ...
            // Usar increment() y update() por separado para evitar errores con DB::raw en update()
            ...
            // Actualizar estadísticas para character1 usando increment() para evitar problemas con DB::raw
            ...
            $characterSurvey1Pivot->update(['updated_at' => now()]); // Actualizar updated_at
            ...
            // Actualizar estadísticas para character2 usando increment() para evitar problemas con DB::raw
            ...
            $characterSurvey2Pivot->update(['updated_at' => now()]); // Actualizar updated_at
