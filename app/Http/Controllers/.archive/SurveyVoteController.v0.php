...
Log::info('Vote created', ['id' => $ok->id]);
            ...
            $ok = $combinatoric->update(['last_used_at' => now()]);
Log::info('Combinatoric updated', ['ok' => $ok, 'id' => $combinatoric->id]);
...
Log::info('SurveyUser updated', ['ok' => $ok, 'user_id' => $surveyUserPivot->user_id, 'survey_id' => $surveyUserPivot->survey_id,'surveyUserPivot' => $surveyUserPivot]);

            // Actualizar la entrada pivote
//             $ok =$surveyUserPivot->update([
//                 'total_votes' => $newTotalVotes,
//                 'progress_percentage' => $newProgressPercentage,
//                 'last_activity_at' => now(),
//                 // 'is_completed' y 'completed_at' se pueden actualizar aquí si se completa, o en otro proceso.
...
// dump($surveyUserPivot);
            ...
            $ok = DB::table('character_survey')
                ->where('character_id', $character1Id)
                ->where('survey_id', $surveyData->id)
                ->update($updateDataChar1); 
Log::info('CharacterSurvey1 updated', ['ok' => $ok, 'id' => $updateDataChar1]);
...
Log::info('CharacterSurvey2 updated', ['ok' => $ok, 'id' => $updateDataChar2]);
// dd($characterSurvey1Pivot, $characterSurvey2Pivot, $updateDataChar1, $updateDataChar2);
